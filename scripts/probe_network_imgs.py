#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""5브랜드 상세페이지 네트워크 이미지 요청 캡처 -> 본문 이미지 유무 판별 (수정)"""
import json, re
...
with open(f"{BASE}/public/data/products.json",encoding="utf-8") as f:
    data=json.load(f)
targets={}
for brand in ["청호나이스","쿠쿠","웰스","세스코","현대큐밍"]:
...
with sync_playwright() as p:
    ...
    for brand,url in targets.items():
        ...
        try:
            ...
            print(f"[{brand}] 로드실패 {e}"); continue
        # 본문 후보: upload/product, goods, detail, desc, editor, /a/ch/prd, prd 경로
