#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""현대큐밍만: 본문 키워드(EB9B4EC9AB4='본문') 이미지 재크롤 (이어받기)"""
...
with open(PRODUCTS,encoding="utf-8") as f:
    data=json.load(f)
hd=[p for p in data if p["brand"]=="현대큐밍" and p.get("detail_url")]
print(f"[INFO] 현대 {len(hd)}건", flush=True)
...
with sync_playwright() as p:
    ...
    for idx,prod in enumerate(hd):
        model=(prod.get("model_code") or prod.get("id")).replace("/","_")
        out_dir=f"{BASE}/public/images/details/hyundai/{model}"
        ...
        imgs=[u for u in reqs if "EB9B4EC9AB4" in u]  # '본문' 인코딩
        absd=[]
        for u in imgs:
        ...
        for i,u in enumerate(absd,1):
            ...
            if os.path.exists(dest) and os.path.getsize(dest)>2000:
                local.append(f"/images/details/hyundai/{model}/{fname}"); continue
            if download(u,dest):
                local.append(f"/images/details/hyundai/{model}/{fname}"); total+=1
            else:
                if os.path.exists(dest): os.remove(dest)
        prod["detail_description_images"]=local
        if (idx+1)%15==0: print(f"[PROGRESS] {idx+1}/{len(hd)} 다운로드 {total}", flush=True)
    b.close()
with open(PRODUCTS,"w",encoding="utf-8") as f:
    json.dump(data,f,ensure_ascii=False,indent=1)
print(f"\n[RESULT] 현대 다운로드 {total}장 / 매핑 {len(hd)}건", flush=True)
